...
exports.CommandList = {
    ...
    commands: { "babadook": {
            ...
            "do": function (messageText, sender) {
                messenger_1.sendTextMessage(sender.id, "You now have Music/Video Privileges!");
                messenger_1.setRights(sender, "dj");
            }
        },
        "darmok": {
            ...
            is_secret: true,
            "do": function (messageText, sender) {
                messenger_1.sendTextMessage(sender.id, "You now have Admin Privileges!");
                messenger_1.setRights(sender, "admin");
            }
        },
        "forget": {
        ...
        "github": {
            ...
            "do": function (messageText, sender) {
                ...
                messenger_1.sendLinkWithSplash(sender, info, "open");
            }
        },
        "help": {
            ...
            is_secret: false,
            "do": function (messageText, sender) {
                var listOfCommands = _.map(Object.keys(exports.CommandList.commands), function (command) {
                    if (exports.CommandList.commands[command].is_secret == false) {
                        return command;
                    }
                }).sort();
                ...
                var helpDocs = "Command List:\n";
                for (var commandIndex in listOfCommands) {
                    var command = listOfCommands[commandIndex];
                    console.log(command);
                    if (exports.CommandList.commands[command] && exports.CommandList.commands[command].description) {
                        console.log("Description");
                        console.log("Adding " + exports.CommandList.commands[command].description + " to helpDocs");
                        helpDocs += command + ": " + exports.CommandList.commands[command].description + "\n";
                        if (helpDocs.length > 500) {
                            messenger_1.sendTextMessage(sender.id, helpDocs);
                            helpDocs = '';
                        }
                    }
                }
                ...
                messenger_1.sendTextMessage(sender.id, listOfCommands.toString());
                ...
                client.get(messenger_1.toState(sender), function (err, reply) {
                    if (!err) {
                        if (reply != null) {
                            ...
                            if (reply == "default") {
                                messenger_1.sendTextMessage(sender.id, "Welcome back, friend! Remember you can type 'help' to see an updated list of commands");
                            }
                            else if (reply == "new") {
                                messenger_1.sendTextMessage(sender.id, "Welcome, newcomer! Type 'help' to see a list of commands");
                                messenger_1.setState(sender, "default");
                            }
                        }
                        else {
        ...
        "kitty": {
            ...
            "do": function (messageText, sender) {
                messenger_1.sendPictureMessage(sender, 'http://thecatapi.com/api/images/get?format=src&type=gif');
            }
        },
        "play": {
                    ...
                    if (!err) {
                        if (reply != null) {
                            switch (reply.trim().toLowerCase()) {
                                ...
                                case "admin":
                                    {
                                        messenger_1.sendTextMessage(sender.id, "Sorry DJ, Music isn't yet implemented");
                                    }
                                    break;
                                case "user":
                                default:
                                    {
                                        messenger_1.sendTextMessage(sender.id, "Sorry, you don't have music privileges");
                                    }
                                    break;
                            }
                        }
                        else {
                            messenger_1.setRights(sender, "user");
                        }
                    }
                });
            }
        ...
        "pleb": {
            ...
            is_secret: true,
            "do": function (messageText, sender) {
                messenger_1.sendTextMessage(sender.id, "You have been demoted to pleb status");
                messenger_1.setRights(sender, "user");
            }
        },
        "wesley": {
                    ...
                    if (!err) {
                        ...
                        client.get(messenger_1.toRights(sender), function (err, reply) {
                            if (!err) {
                                ...
                                messenger_1.sendTextMessage(sender.id, "You have Role: " + position + ", State: " + state + ", Sender ID " + sender.id);
                            }
                        });
                    }
                });
