...
function receivedMessage(event : Event) {
 ...
 *
 */
function receivedPostback(event : Event) {
    var senderID = event.sender.id;
    var recipientID = event.recipient.id;
 ...
 *
 */
function sendTextMessage(recipientId : string, messageText : string) {
    var messageData = {
        recipient: {
            id: recipientId
        },
...
function parseQuickReply(quickReply : QuickReply) {
  return 0;
}
// Start server
